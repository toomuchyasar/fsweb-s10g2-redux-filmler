...
const Movie = (props) => {
  ...
  const display = useSelector((store) => store.favoriteReducer.displayFavorites)
  ...
  return (
    <div className="bg-white rounded-md shadow flex-1">
      ...
      <div className="px-5 py-3 border-t border-zinc-200 flex justify-end gap-2">
        <button onClick={() => handleDelete()} type="button" className="myButton bg-red-600 hover:bg-red-500">Sil</button>
        <button onClick={() => handleFav()} className="myButton bg-blue-600 hover:bg-blue-500 ">Favorilere ekle</button>
      </div>
    </div>
  );
}
