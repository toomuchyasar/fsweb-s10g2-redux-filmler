...
const FavoriteMovieList = (props) => {
  const favorites = useSelector((movie) => movie.favoriteReducer.favorites)
      ...
      <div className="pt-3 text-sm">
        {
          favorites.map(movie => (
            <Link key={movie.id} className="py-1 flex gap-2 justify-between" to={`/movies/${movie.id}`}>
              {movie.title}
              <span onClick={handleDelFav} className="material-icons hover:text-red-600 text-[18px]">remove_circle</span>
            </Link>
          ))
        }
      </div>
